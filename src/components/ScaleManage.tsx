...
const MetricCard = ({ label, value, trend, trendUp }: { label: string, value: string, trend: string, trendUp: boolean }) => (
  <div className="bg-[#0A0A0A] border border-white/5 rounded-xl p-5 flex flex-col justify-between h-32 relative overflow-hidden group">
    ...
    <div className="z-10">
      ...
      <div className={cn("text-xs flex items-center gap-1", trendUp ? "text-emerald-400" : "text-emerald-400")}>
...
const AnalyticsView = () => (
  <div className="absolute inset-0 p-8 flex flex-col bg-[#050505]">
    <h3 className="text-lg font-medium text-white mb-6">Analytics & Insights</h3>
    
    {/* Metrics Grid */}
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
      <MetricCard label="Total Conversations" value="12,847" trend="+12.5%" trendUp={true} />
      <MetricCard label="AI Resolution Rate" value="78.2%" trend="+3.1%" trendUp={true} />
      <MetricCard label="Avg Response Time" value="1.4s" trend="-0.3s" trendUp={true} />
      <MetricCard label="Customer Satisfaction" value="4.7/5" trend="+0.2" trendUp={true} />
    </div>
            ...
            <div key={i} className="grid grid-cols-4 text-sm items-center group">
              ...
              <div className="flex items-center gap-3">
                <div className="h-1.5 w-16 sm:w-24 bg-gray-800 rounded-full overflow-hidden">
                  <div className="h-full bg-orange-500 rounded-full" style={{ width: `${row.sat}%` }} />
                </div>
                <span className="text-gray-400 text-xs hidden sm:inline">{row.sat}%</span>
              </div>
            </div>
          ))}
        </div>
...
const IntegrationCard = ({ icon: Icon, name, desc, count, color, connected }: any) => (
...
const IntegrationsView = () => (
  <div className="absolute inset-0 p-8 flex flex-col bg-[#050505]">
    <h3 className="text-lg font-medium text-white mb-6">Available Integrations</h3>
    
    {/* Filters */}
    <div className="flex gap-2 mb-8 overflow-x-auto pb-2 scrollbar-hide">
      {["All", "AI Tools", "Communication", "CRM", "Storage"].map((filter, i) => (
        <button 
          key={i} 
          className={cn(
       ...
       <div className="absolute inset-0 flex items-center justify-center scale-75 md:scale-100 origin-center">
          ...
          <div className="absolute left-12 top-1/2 -translate-y-1/2 w-48 bg-[#111] border border-white/10 rounded-xl p-4 shadow-xl z-10">
             <div className="flex items-center gap-3 mb-2">
                <div className="w-8 h-8 rounded bg-blue-500/20 flex items-center justify-center text-blue-400"><Zap size={16} /></div>
          ...
          <svg className="absolute inset-0 pointer-events-none z-0">
             <path d="M 240 250 C 300 250, 300 250, 360 250" stroke="#333" strokeWidth="2" fill="none" />
          </svg>
          ...
          <div className="absolute left-[360px] top-1/2 -translate-y-1/2 w-48 bg-[#111] border border-orange-500/30 rounded-xl p-4 shadow-[0_0_30px_rgba(203,52,42,0.1)] z-10">
             <div className="flex items-center gap-3 mb-2">
                <div className="w-8 h-8 rounded bg-orange-500/20 flex items-center justify-center text-orange-400"><Cpu size={16} /></div>
          ...
          {/* Connection 2 Split */}
          <svg className="absolute inset-0 pointer-events-none z-0">
             <path d="M 550 250 C 600 250, 600 180, 650 180" stroke="#333" strokeWidth="2" fill="none" />
             <path d="M 550 250 C 600 250, 600 320, 650 320" stroke="#333" strokeWidth="2" fill="none" />
          </svg>

          {/* End Nodes */}
          <div className="absolute left-[650px] top-[140px] w-48 bg-[#111] border border-white/10 rounded-xl p-4 shadow-xl z-10 opacity-60">
             <div className="flex items-center gap-3 mb-2">
                <div className="w-8 h-8 rounded bg-green-500/20 flex items-center justify-center text-green-400"><Slack size={16} /></div>
          ...
          <div className="absolute left-[650px] top-[280px] w-48 bg-[#111] border border-white/10 rounded-xl p-4 shadow-xl z-10 opacity-60">
             <div className="flex items-center gap-3 mb-2">
                <div className="w-8 h-8 rounded bg-purple-500/20 flex items-center justify-center text-purple-400"><Mail size={16} /></div>
            ...
            >
              <div className="h-1 w-full bg-white/10 rounded-full overflow-hidden">
                {activeIndex === index ? (
                  ...
                  <div className={cn("h-full bg-orange-500 w-0", index < activeIndex && "w-full")} />
                )}
              </div>
              <div>
